from metaflow import FlowSpec, step, Parameter, kubernetes, resources, timeout, retry, catch, conda_base
import pandas as pd
import mlflow
import numpy as np
import os
...
class WineScoringFlow(FlowSpec):
    ...
    def start(self):
        ...
        """
        if self.error:
            print(f"Error caught in start step: {self.error}")
            
        print(f"Starting the Wine Scoring Flow for model: {self.model_name}")
        
        # Set up MLFlow with the running server
        mlflow.set_tracking_uri("https://mlflow-service-514897228720.us-west2.run.app/")
    ...
    def process_input(self):
        ...
        """
        if self.error:
            print(f"Error caught in process_input step: {self.error}")
            
        print("Processing input data")
        
        # If no input_data_path provided or can't access it, use scikit-learn dataset
        if self.input_data_path is None:
            print("No input data path provided, using scikit-learn test data")
            from sklearn import datasets
            wine = datasets.load_wine()
            X = pd.DataFrame(wine.data, columns=wine.feature_names)
            y = pd.Series(wine.target)
            self.input_data = pd.concat([pd.DataFrame(y, columns=['class']), X], axis=1)
            print(f"Loaded scikit-learn wine dataset with shape: {self.input_data.shape}")
        else:
            try:
                ...
                print(f"Error loading data from {self.input_data_path}: {e}")
                print("Falling back to scikit-learn test data")
                from sklearn import datasets
                wine = datasets.load_wine()
                X = pd.DataFrame(wine.data, columns=wine.feature_names)
                y = pd.Series(wine.target)
    ...
    def make_predictions(self):
        ...
        """
        if self.error:
            print(f"Error caught in make_predictions step: {self.error}")
    ...
    def end(self):
        ...
        """
        if self.error:
            print(f"Error caught in end step: {self.error}")
